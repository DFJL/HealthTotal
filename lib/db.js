...
export async function getBodyMeasurements(userId) {
  ...
  return data.map(r => ({
    ...
    note: r.note, isSeed: r.is_seed, source: r.source || 'inbody',
  }));
}
...
export async function getLabResults(userId) {
  ...
  if (error) throw error;
  // Reconstruct flat format from stored markers JSONB
  return data.map(r => ({
    date: r.tested_at,
    summary: r.notes,
    ...(r.markers || {}),
...
export async function insertLabResult(userId, lab) {
  const { summary, date, panel, markers, notes, ...flatMarkers } = lab;
  const { error } = await supabase
    .from('lab_results')
    .insert({
      user_id: userId,
      tested_at: lab.date,
      panel_name: lab.panel || 'Panel',
      markers: flatMarkers,
      notes: lab.summary || lab.notes || null,
    });
  if (error) throw error;
}

export async function insertLabResultsBatch(userId, labs) {
...
export async function insertBodyMeasurementsBatch(userId, rows) {
  ...
  const { error } = await supabase.from('body_measurements')
    .upsert(data, { onConflict: 'user_id,measured_at', ignoreDuplicates: true });
  if (error) throw error;
}
...
export async function getAiCache(userId, cacheKey) {
  const { data, error } = await supabase
    ...
    .maybeSingle();
  if (error || !data) return null; // not found
